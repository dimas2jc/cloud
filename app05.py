...
def create_mongodatabase():
    try:
        ...
        if 'app' not in dbnames:
            db_users = connection.app.users
            db_tweets = connection.app.tweets
            db_users.insert( {
...
def add_tweets():
    tweets = connection.app.tweets
    user_tweet = {}
    if not request.json or not 'username' in request.json or not 'body' in request.json:
        abort(400)
    user_tweet['tweetedby'] = request.json['username']
    user_tweet['body'] = request.json['body']
    user_tweet['timestamp']=datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
...
def add_tweet(new_tweet):
    ...
    user = users.find({"username":new_tweet['tweetedby']})
